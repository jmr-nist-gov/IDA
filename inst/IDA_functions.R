IDA_parse <- function(dat) {
  ...
  if (length(sample_breaks) > 1) {
    for (i in 2:length(sample_breaks)) {
      fn_obj[i - 1] <- list(
        dat %>%
          slice((sample_breaks[i - 1] + 2):(sample_breaks[i] - 1)) %>%
          as.list()
      )
...
IDA_stable <- function(dat, buffer = 10, tolerance = 0.1, expansion = 20) {
  ...
  while (difference <= tolerance & exit_check) {
    temp <- interest_region[
      c((start_point - backward):(start_point + forward)),
    ]
...
IDA_graphs <- function(
  ...
  signal <- dat_raw %>%
    ...
    ggplot(aes(x = Time)) +
    geom_hline(
      yintercept = attr(dat_rat, "iso1.background"),
      colour = 'black'
    ) +
  ...
  if (packageVersion("scales") >= "1.2") {
    signal <- signal +
      scale_x_continuous(
        labels = label_number(scale = 0.001, big.mark = ","),
        n.breaks = 10
      )
  }
  ...
  ratios <- dat_rat %>%
    ...
    geom_line() +
    labs(x = "Time (s)", y = paste("Signal Ratio", isotopes)) +
...
IDA <- function(
  ...
  for (i in 1:length(dat)) {
    ...
    out <- rbind(
      out,
      IDA_summary(temp, attr(temp, "stable.start"), attr(temp, "stable.end"))
    )
    info <- rbind(
      info,
      data.frame(
        Isotope1 = attr(temp, "iso1.background"),
...
pack_as_excel <- function(
  IDA_obj,
  draw_stable_bounds = FALSE,
  formatter = lubridate::mdy_hms
) {
  wb <- createWorkbook()
  ...
  for (i in 1:length(IDA_obj$Processed)) {
    ...
    temp <- data.frame(cbind(proc$Time, proc$Ratio, proc[, 2:3], raw))
    nam_list <- c(
      "Time",
      "Ratio",
    ...
    if (draw_stable_bounds) {
      print(
        IDA_obj$Graphs[[i]]$Signal +
          geom_vline(
            xintercept = IDA_obj$Info$`Stable Time Start`[i],
            colour = 'red'
          ) +
          geom_vline(
            xintercept = IDA_obj$Info$`Stable Time End`[i],
            colour = 'red'
          )
      )
    } else {
